...
func (e *VehicleType) GetExcel(list []models.VehicleType) ([]byte, error) {
	...
	_ = xlsx.SetSheetRow(sheetName, "A1", &[]interface{}{"创建者", "创建时间", "主键，自动递增", "车辆种类名称 (如“小型汽车”)", "更新者", "更新时间"})
	// dictService := adminService.NewSysDictDataService(&e.Service)
	for i, item := range list {
		axis := fmt.Sprintf("A%d", i+2)
		//按标签对应输入数据
		_ = xlsx.SetSheetRow(sheetName, axis, &[]interface{}{item.CreateBy, item.CreatedAt, item.Id, item.TypeName, item.UpdateBy, item.UpdatedAt})
	}
